...
export async function extractCluecoords2(output: ScriptOutput, fs: ScriptFS, filesource: CacheFileSource) {
    let enums: number[] = [];
    let itemindex = await filesource.getCacheIndex(cacheMajors.items);
    for (let index of itemindex) {
        let files = await filesource.getFileArchive(index);
    ...
    allcoords.forEach((q, i) => {
        let clueid = idmapping[i];

        if(!indexedcoords[clueid]) indexedcoords[clueid] = []

        indexedcoords[clueid] = q.map(c => ({x: c.x, y: c.z, level: c.level as floor_t}))
    })
